-- =============================================================================
-- POS 3B — Backfill: recolección 2026-08-22 → préstamos área Por recolectar
-- Ejecutar en Supabase → SQL Editor DESPUÉS de fix_prestamos_interarea_rc_virtual.sql
-- =============================================================================
-- Marca préstamos ligados a cortes RECOLECCION del 22/08/2026 como por_recolectar,
-- sella colectado_* si faltaba y reabre los «recuperado» de ese día que no pasaron
-- por RC Virtual, para poder Ajustar / Recolectar en Vales y Préstamos.

do $$
declare
  v_fecha date := date '2026-08-22';
  v_ini timestamptz := (v_fecha::text || ' 00:00:00')::timestamptz;
  v_fin timestamptz := (v_fecha::text || ' 23:59:59.999')::timestamptz;
begin
  -- 1) Sellar colecta desde cierres de recolección del día (por gasto_id)
  ...
  -- 2) Sellar por token PRESTAMO-IA en el detalle del cierre
  update public.prestamos_interarea p
  set
    colectado_por = coalesce(p.colectado_por, c.usuario_nombre, 'Recolector'),
    colectado_at = coalesce(p.colectado_at, c.created_at),
  ...
  -- 3) Reabrir solo los del 22/08 (fecha, colecta ese día, o ya sellados arriba)
  update public.prestamos_interarea p
  set
    ...
    colectado_por = coalesce(p.colectado_por, 'Admin · backfill 2026-08-22'),
    colectado_at = coalesce(p.colectado_at, v_ini + interval '12 hours'),
    colectado_folio = coalesce(p.colectado_folio, 'BACKFILL-2026-08-22'),
  ...
  where coalesce(p.rc_recibido_por, '') = ''
    ...
    and (
      ...
      or coalesce(p.colectado_folio, '') like 'REC-%'
         and p.colectado_at is not null
         and p.colectado_at >= v_ini
         and p.colectado_at <= v_fin
    );

  -- 4) Abiertos con saldo y colecta del día → por_recolectar
  update public.prestamos_interarea p
...
where fecha = date '2026-08-22'
   or (
     colectado_at >= timestamptz '2026-08-22 00:00:00'
     and colectado_at <= timestamptz '2026-08-22 23:59:59.999'
   )
   or colectado_folio = 'BACKFILL-2026-08-22'
