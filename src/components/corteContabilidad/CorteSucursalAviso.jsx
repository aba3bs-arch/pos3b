...
import { esSucursalClienteMaquinas } from '../../lib/clientesMaquinas.js';
...
export default function CorteSucursalAviso({ sucursal, user }) {
  ...
  const esSocio = esSucursalClienteMaquinas(sid);
  const puedeCambiar = puedeCambiarTiendaLibremente(user?.rol);

  return (
    <div style={{ marginBottom: '0.75rem' }}>
      <span className="badge" style={{ fontSize: '0.82rem' }}>
        {esSocio ? 'Espacio Socio 3B' : 'Tienda del corte'}: {etiquetaTienda(sucursal)}
      </span>
      {esSocio ? (
        <p className="muted" style={{ margin: '0.5rem 0 0', fontSize: '0.82rem' }}>
          Cortes Virtual y Garage de este socio se guardan en la sucursal sintética <strong>{sid}</strong> (aislados de
          las tiendas 3B). Sin alertas de recuperación.
        </p>
      ) : null}
      {enMain && puedeCambiar && !esSocio && (
        <p className="muted" style={{ margin: '0.5rem 0 0', fontSize: '0.82rem' }}>
