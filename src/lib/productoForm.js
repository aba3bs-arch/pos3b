...
export function productoParaGuardar(form, opts = {}) {
  ...
  const base = {
    ...
    precio_compra_sin: compraSin,
    precio_compra_con: compraCon,
    precio_ruta: round2(form.precio_ruta || 0),
    costo: compraCon,
  ...
  const mapFav = (() => {
    ...
    if (sucursal) {
      ...
      if (suc && suc !== 'MAIN') map[suc] = Boolean(form.en_favoritos);
    }
    return map;
  })();
 ...
 * - Sin mapa (legado): en_favoritos / cat FAVORITOS aplica a todas.
 */
 ...
 * Legado en_favoritos=true → todas las sucursales POSibles en true.
 */
